...
public class CalculateurValidator extends CalculSwitch<Boolean> {
	...
	public Boolean caseBinaire(Binaire object) {
		boolean deuxPred = object.getLinkToPredecessor().stream().count() == 2;
		
		boolean unSuc = object.getLinkToSuccessor().stream().count() == 1;
		
		this.result.recordIfFailed(
	            deuxPred,
	            object,
	            "L'opération binaire n'a pas exactement deux entrées.");
		
		this.result.recordIfFailed(
	            unSuc,
	            object,
	            "L'opération binaire n'a pas exactement une sortie");
