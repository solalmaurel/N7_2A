 ...
 * @version 0.1
 */
public class ValidateCalculateur {
	...
	private static void afficherResultat(ValidationResult resultat) {
		afficherErreurs("- UnCalcul", resultat.getRecordedErrorsFor(CalculPackage.UN_CALCUL));
		afficherErreurs("- Entree", resultat.getRecordedErrorsFor(CalculPackage.ENTREE));
		afficherErreurs("- Sortie", resultat.getRecordedErrorsFor(CalculPackage.SORTIE));
		afficherErreurs("- Constante", resultat.getRecordedErrorsFor(CalculPackage.CONSTANTE));
		afficherErreurs("- Binaire", resultat.getRecordedErrorsFor(CalculPackage.BINAIRE));
		afficherErreurs("- Unaire", resultat.getRecordedErrorsFor(CalculPackage.UNAIRE));
		afficherErreurs("- Link", resultat.getRecordedErrorsFor(CalculPackage.LINK));
		afficherErreurs("- Expression", resultat.getRecordedErrorsFor(CalculPackage.EXPRESSION));
	...
	public static void main(String... args) {
		...
		CalculateurValidator validator = new CalculateurValidator();
