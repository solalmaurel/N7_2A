...
(******************************************************************************)
let rec retrait_arbre c (Noeud (b, lb)) =
   match recherche c lb with
      | None -> let lb_maj = List.map (fun (tc, ta) -> (tc, retrait_arbre c ta)) lb in
      Noeud(b, lb_maj)
      | Some _ -> let lb_filter = List.filter(fun (tc, _) -> tc <> c) lb in
      let lb_maj = List.fold_right (fun (tc, ta) acc -> maj tc ta acc) lb_filter [] in
      Noeud(b, lb_maj)

(******************************************************************************)
(*   fonction qui renvoie la liste des éléments dans l’ordre                  *)
(*   lexicographique d’un arbre                                               *)
(*   signature  : parcours : 'a arbre -> 'a list                              *)
(*   paramètres : - un arbre n-aire                                           *)
(*   résultat   : a liste des éléments dans l’ordre lexicographique           *)
(******************************************************************************)
let parcours_arbre (Noeud(_, brs)) =
   let rec parcours_branches brs =
      match brs with
      | [] -> []
      | (c,Noeud(b,ss_brs))::q ->
      let ss_liste = parcours_branches ss_brs
      and q_liste = parcours_branches q
      in let liste = (List.map (fun l -> c::l) ss_liste)@q_liste in
      if b then [c]::liste esle liste
   in (if b then [[]] else [])@(parcours_branches brs)


let rec parcours_arbre (Noeud(b, brs)) =
   let cons t q = t::q in
   let parcours_branches (c, a) = List.map (cons c) (parcours_arbre a)
   in let ss_lsite = List.Flatten (List.map parcours_branches brs)
   in if b then []::ss_liste else ss_liste
