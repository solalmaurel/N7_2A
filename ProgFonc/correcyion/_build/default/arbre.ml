...
struct
  ...
  let rec conforme a =
    match a with
      | Vide -> true       
      | Noeud (b, fils) ->  let rec liste_conforme l =
                              match l with
                                | [] -> true
                                | t::q -> if conforme t then
                                            ...
                                            false
                            in
                              (List.length fils) = B.base && liste_conforme fils
    ...
    | Vide, _ -> false
    | _, [] -> false
    | Noeud (_, fils), t::q -> if q==[] then
      match get t fils with
      | Vide -> false
      | Noeud (f, _) -> f
    else appartient q (get t fils)
      ...
      match l with
        | [] -> (
                  match a with
                    ...
                    | Noeud (b, fils) -> Noeud (true, fils)
                )
        | t::q -> (
                    match a with
                      | Vide -> ajouter l (Noeud (false, l_taille B.base Vide))
                      | Noeud (b, fils) -> Noeud (b, set t fils (ajouter q (get t fils)))
                  )
    ...
    let rec retirer l a =
      match a with
        ...
        | Noeud (b, fils) ->  ( 
                                match l with
                                  | [] -> if fils = l_taille B.base Vide then
                                            Vide
                                          else
                                            Noeud (false, fils)
                                  | t::q -> let a_sans_l = Noeud (b, set t fils (retirer q (get t fils))) in
                                              if a_sans_l = Noeud (false, l_taille B.base Vide) then
                                                Vide
                                              else
                                                a_sans_l
                              )
