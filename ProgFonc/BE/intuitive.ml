...
struct
  ...
  let encoder_lettre lettre =
    let rec encode_aux lettre ts =
    match E.map with
    | [] -> 0
    | (touche, lettres)::ts -> if List.mem  lettre lettres then touche else encode_aux lettre ts
    in encode_aux lettre E.map
  ...
  let encoder_mot mot = match mot with
  | "" -> []
  | t::q -> (encoder_lettre t)@encoder_mot q
