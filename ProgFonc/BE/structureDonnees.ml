...
struct
  ...
  let rec ajouter enc mot dico = match enc, dico with
  | [], Noeud (mots, enfants) -> Noeud (mot::mots, enfants)
  | e::suite, Noeud (mots, enfants) -> let rec ajout_enfant enfants = match enfants with
        | [] -> [(e, ajouter suite mot empty)]
        | (f, fils) :: q when fils = e -> (f, ajouter reste mot fils) :: q
        | hd :: q -> hd :: ajout_enfant q
      in
      Noeud (mots, ajout_enfant enfants)
